...
public class GetHonorByIdEventHandler : IRequestHandler<GetHonorByIdEvent,ActionResult>
{
  ...
  public async Task<ActionResult> Handle(GetHonorByIdEvent request, CancellationToken cancellationToken)
  {
    ...
    if (honor == null) return new NotFoundObjectResult(new FanResponse("Not found", ResponseStatus.Error));

    return new OkObjectResult(new CoachHonorDto(honor.Id,honor.TrophyId, request.Lang == Languages.En ? honor.Trophy.Name : honor.Trophy.NameAr, 
